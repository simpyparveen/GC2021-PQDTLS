...
/** Reading Handshake messages*/
    while (1)
    {
        ...
        FD_SET(fd, &rfds);           //FD_SET(fd, &rfds); //fd =socket ()
        ...
        if (result < 0) {
            ...
            if (FD_ISSET(filefd, &rfds)) {
                ...
                if (bytes_read == 0) {
                   dtls_ticks(&stop_f);
		   t2 = clock() - t1;
                    printf("Time sending file %d ticks and %lf seconds(CLOCKS_PER_SEC=%d)",(stop_f-start_f),(double)t2/CLOCKS_PER_SEC,CLOCKS_PER_SEC);
                    //dtls_ticks(&stop_rtt1);
                    //rtt1 = (double)(stop_rtt1 - start_rtt1) / (dtls_tick_t)DTLS_TICKS_PER_SECOND; //DTLS_TICKS_PER_SECOND =1000
                    //printf("RTT for sending file  %lf", rtt1);
                    break;
                }
                len += bytes_read;
                printf("Len: %d\n", len);
