//
// Created by Simpy on 30/10/19.
//

//
// Created by Simpy Parveen on 27/10/19.
//
...
#include <string.h>
#include <assert.h>
//#include "ecc/tests/test_helper.c"
//#include "ecc/ecc.h"
#include "ecc/ecc.c"
...
static const uint32_t ecdsaTestMessage[] = { 0x65637571, 0x20612073, 0x68206F66, 0x20686173, 0x69732061, 0x68697320, 0x6F2C2054, 0x48616C6C};
...
int main(int argc, char const *argv[])
{
    ...
    uint8_t pkx[32];//public key x
    uint8_t pky[32];//public key y
    //ecc_ec_mult(BasePointx, BasePointy, ecdsaTestSecret, pub_x, pub_y);  //Public key, Q(x,y) = d * G(x,y) (private key :d, base point G)
    dtls_ecdsa_generate_key(sk,pkx,pky,32);


/**
 *
int ecc_ecdsa_sign_hash(const uint32_t *ecdsaTestSecret, const uint32_t *ecdsaTestMessage, const uint32_t *ecdsaTestRand1, uint32_t *tempx, uint32_t *tempy)

 * Calculate the ecdsa signature.

 * input:
 *  ecdsaTestSecret: private key on the curve secp256r1 (32 bytes)
 *  ecdsaTestMessage: hash to sign (32 bytes)
 *  ecdsaTestRand1: random data, this must be changed for every signature (32 bytes)
 *
 * output:
 *  tempx: r value of the signature (36 bytes)
 *  tempy: s value of the signature (36 bytes)
 *
 * return:
 *   0: everything is ok
 ...
 */
    ...
    dtls_hash_init(&data);
    dtls_hash_update(&data, client_random, sizeof(client_random));
    dtls_hash_update(&data, server_random, sizeof(server_random));
    dtls_hash_finalize(sha256hash, &data);

    dtls_ec_key_to_uint32(sha256hash,32, hash); //#define DTLS_HMAC_DIGEST_SIZE 32

    dtls_ec_key_to_uint32(secret,32, sk);
    dtls_ec_key_to_uint32(pub_x,32, pkx);
    dtls_ec_key_to_uint32(pub_y,32, pky);
    ret = ecc_ecdsa_sign_hash(sk, hash, ecdsaTestRand1, tempx, tempy);//(x1, y1) = k × G(x, y) mod p , r = x1 mod n, s = (k-1 (h(m) + d * r) mod n
 ...
 *int ecc_ecdsa_validate(const uint32_t *pub_x, const uint32_t *pub_y, const uint32_t *ecdsaTestMessage, const uint32_t *tempx, const uint32_t *tempy)
 *
 *
 * Verifies a ecdsa signature.

 * input:
 *  pub_x: x coordinate of the public key (32 bytes)
 *  pub_y: y coordinate of the public key (32 bytes)
 *  ecdsaTestMessage: hash to verify the signature of (32 bytes)
 *  tempx: r value of the signature (32 bytes)
 *  tempy: s value of the signature (32 bytes)
 *
 * return:
 *  0: signature is ok
